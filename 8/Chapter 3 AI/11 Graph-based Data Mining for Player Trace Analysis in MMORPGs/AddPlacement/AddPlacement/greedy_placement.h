#ifndef GREEDY_PLACEMENT_H
#define GREEDY_PLACEMENT_H
...
void greedy_compute_placement(int placement_count, WalkDb &db, set<int> &training, set<int> testing, double &training_coverage, double &testing_coverage)
{
	...
	for(int i = 0; i < placement_count; i++)
	{
		...
		if (old_best.size() == best_set.size())
		{
			set<int> set_to_try = old_best;
			best_set.insert(*(vertices_left.begin()));
			vertices_left.erase(vertices_left.begin());
		}
		else
...
void greedy_compute_placement_stat(int placement_count, 
								   WalkDb &db,  
								   ...
								   double &dev_testing_coverage)
{
	...
	for(int j = 0; j < MAX_RANDOM_KEYS; j++)
	{
		...
		greedy_compute_placement(placement_count, db, training, testing, curr_training_coverage, curr_testing_coverage);
