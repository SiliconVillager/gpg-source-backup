...
void SHADER::Init(IDirect3DDevice9 *Dev, const char fName[], int typ)
{
	...
	{
		if(m_type == PIXEL_SHADER)
			s_log.Print("Couldnt compile the Pixel Shader");
		else s_log.Print("Couldnt compile the Vertex Shader");

		s_log.Print((char*)ErrorMsgs->GetBufferPointer());
		return;
	}
	...
	{
		if(m_type == PIXEL_SHADER)
			s_log.Print("Couldnt Create the Pixel Shader");
		else s_log.Print("Couldnt Create the Vertex Shader");
		exit(0);
	}
}
